package com.kindleassistant;

public class AppConstants {
	public static String BASE_URL = "http://114.215.110.96/V1";
	public static String TEST_URL = "http://114.215.110.96/test";
	public static String SEND_URL = BASE_URL+"/send/url";
	public static String PREVIEW_URL = BASE_URL+"/send/preview/";
	public static String UPLOADS_URL = BASE_URL+"/uploads";
	public static String UPDATE_EMAIL = BASE_URL+"/user/update_email/";
	public static String USER_CREATE = BASE_URL+"/user/create/";
}
